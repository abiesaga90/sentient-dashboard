...
export function MetalsSubTab({ rows }: Props) {
  const cols: Column<TokenizedRow>[] = [
    {
      ...
      render: (r) => (
        <div className="flex items-center gap-2">
          ...
          {r.crypto_native && (
            <Badge variant="info" className="text-[10px]">on-chain</Badge>
          )}
        </div>
      ),
    },
    ...
    },
    {
      ...
      header: "Spot Ref",
      align: "right",
      sortKey: (r) => r.fundamentals.spot_ref_usd ?? -Infinity,
      render: (r) => (
        <span className="font-mono text-xs text-gray-400">{fmtPrice(r.fundamentals.spot_ref_usd)}</span>
      ),
    },
    {
      key: "premium",
      header: "Premium",
      align: "right",
      sortKey: (r) => r.fundamentals.premium_to_spot_pct ?? 0,
      render: (r) => (
        <span className={`font-mono text-xs ${pctColor(r.fundamentals.premium_to_spot_pct ?? null)}`}>
          {fmtPct(r.fundamentals.premium_to_spot_pct ?? null, 2)}
        </span>
      ),
    },
    {
